...
class SlotScene extends Phaser.Scene{
 ...
 preload(){SYMBOL_KEYS.forEach(k=>this.load.image(k,`assets/symbols/${k}.webp`))}
 create(){scene=this; this.makeGrid(); this.scale.on('resize',()=>this.layout()); this.layout(); this.events.on('shutdown',()=>{this.clearPayline()});}
 makeGrid(){for(let i=0;i<15;i++){const t=this.add.image(0,0,SYMBOL_KEYS[rand()]).setOrigin(.5);this.symbols.push(t);this.grid.push(rand())}}
 layout(){const w=this.scale.width,h=this.scale.height; this.symbols.forEach((t,i)=>{t.x=((i%5)+.5)*w/5;t.y=(Math.floor(i/5)+.5)*h/3;t.setDisplaySize(Math.min(w/5,h/3)*.78,Math.min(w/5,h/3)*.78);});}
 ...
 for(let c=0;c<5;c++)if(!stopped[c])for(let r=0;r<3;r++)values[c+r*5]=rand();this.symbols.forEach((t,i)=>t.setTexture(SYMBOL_KEYS[values[i]]));if(stopped.every(Boolean)){spinning=false;const win=Math.random()<.35?bet*(2+Math.floor(Math.random()*8)):0;if(win){balance+=win;$('win').textContent=win;const row=Math.floor(Math.random()*3);this.showPayline(Math.floor(Math.random()*20));for(let c=0;c<5;c++)this.symbols[c+row*5].setTint(0xc6ff50)}else $('win').textContent=0;setText();return;}this.time.delayedCall(55,tick);};tick();}
}
